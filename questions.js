...
const generateQuestions = () => {
    ...
    // Q1: 데이터베이스 캐싱 전략 (ElastiCache)
    {
        ...
        options: ["지연 로딩(Lazy Loading) 전략을 사용한다.", "라이트 스루(Write-Through) 전략을 사용한다.", "TTL(Time-To-Live) 값만 추가한다.", "AWS AppConfig를 사용하여 데이터를 캐싱한다."],
    ...
    // Q2: 데이터 백업 및 규정 준수 (AWS Backup)
    {
        ...
        options: ["AWS Backup의 볼트 잠금(Vault Lock) 기능을 규정 준수 모드로 사용한다.", "Amazon S3 수명 주기 정책을 사용하여 삭제를 방지한다.", "Amazon Data Lifecycle Manager를 사용하여 스냅샷을 관리한다.", "IAM 권한을 수정하여 모든 사용자의 삭제 권한을 제거한다."],
    ...
    // Q3: API 배포 전략 (API Gateway)
    {
        category: "애플리케이션 통합",
        title: "새로운 버전의 REST API를 배포하려고 합니다. 전체 트래픽의 5%만 새 버전으로 라우팅하여 안정성을 테스트한 후, 점진적으로 트래픽을 늘리고자 합니다. 가장 적절한 배포 방식은 무엇입니까?",
        options: ["API Gateway의 카나리아(Canary) 릴리스 배포를 사용한다.", "블루/그린 배포를 사용하여 트래픽을 한 번에 전환한다.", "모든 사용자를 새 버전으로 즉시 이동시킨다.", "Route 53 가중치 기반 라우팅을 사용하여 API 엔드포인트를 분리한다."],
    ...
    // Q4: AI/ML 문서 처리 (Textract & Comprehend Medical)
    {
        category: "머신러닝",
        ...
        options: ["Amazon Rekognition으로 텍스트를 추출하고 Amazon Translate로 번역한다.", "Amazon Textract로 텍스트를 추출하고 Amazon Comprehend Medical로 의료 정보를 분석한다.", "Amazon EC2에 OCR 소프트웨어를 설치하여 직접 처리한다.", "Amazon Transcribe Medical을 사용하여 텍스트를 변환한다."],
    ...
    // Q5: 데이터베이스 이벤트 처리 (DynamoDB Streams)
    {
        category: "데이터베이스",
        title: "Amazon DynamoDB 테이블에 새로운 항목이 추가될 때마다 해당 데이터를 비동기적으로 처리하여 분석 시스템으로 보내려고 합니다. 운영 오버헤드가 가장 적은 방법은 무엇입니까?",
        options: ["DynamoDB Streams를 활성화하고 AWS Lambda를 트리거하여 처리한다.", "애플리케이션에서 DynamoDB에 쓸 때 동시에 분석 시스템에도 데이터를 보낸다.", "매시간 DynamoDB 테이블을 스캔(Scan)하는 스크립트를 실행한다.", "DynamoDB 데이터를 S3로 내보낸 후 처리한다."],
...
// --- 6번 ~ 35번 문제 추가 ---
    ...
    // Q6: 데이터베이스 마이그레이션 (Aurora & Babelfish) 
    {
        ...
        options: ["Babelfish for Aurora PostgreSQL을 활성화한다.", "AWS Schema Conversion Tool(SCT)만 사용한다.", "RDS Proxy를 사용하여 쿼리를 변환한다.", "DynamoDB로 마이그레이션한다."],
    ...
    // Q7: EKS 비용 최적화 
    {
        ...
        options: ["Amazon EC2 스팟 인스턴스(Spot Instances)", "온디맨드 인스턴스", "전용 호스트(Dedicated Hosts)", "예약 인스턴스(Reserved Instances)"],
    ...
    // Q8: 웹 세션 관리 (ElastiCache) 
    {
        ...
        options: ["Amazon ElastiCache for Redis", "EC2 인스턴스 메모리", "EBS 볼륨", "AWS Systems Manager Session Manager"],
    ...
    // Q9: JSON 데이터 백업 및 복구 
    {
        ...
        options: ["Amazon S3 Standard", "Amazon S3 Glacier Deep Archive", "Amazon RDS for PostgreSQL", "Amazon OpenSearch Service"],
    ...
    // Q10: 데이터베이스 암호화 보안 
    {
        ...
        options: ["AWS KMS를 사용한 클라이언트 측 암호화(Client-side encryption)", "RDS 인스턴스에 대한 TDE(Transparent Data Encryption) 활성화", "SSL/TLS를 사용한 전송 중 암호화", "S3 버킷 암호화 사용"],
    ...
    // Q11: 서버리스 API 구축 
    {
        ...
        options: ["Amazon API Gateway + AWS Lambda + Amazon RDS", "EC2 인스턴스 + 로드 밸런서 + RDS", "ECS 컨테이너 + Fargate + DynamoDB", "API Gateway + Lambda + Redshift"],
    ...
    // Q12: 감사용 데이터 공유 
    {
        ...
        options: ["스냅샷을 암호화하여 복사본을 만든 뒤, 감사자의 AWS 계정과 KMS 키를 공유한다.", "스냅샷을 퍼블릭으로 설정하여 다운로드하게 한다.", "데이터베이스의 사용자 계정을 생성하여 직접 접속하게 한다.", "스냅샷을 S3로 내보내고 버킷 정책으로 모든 접근을 허용한다."],
    ...
    // Q13: 글로벌 게임 서비스 지연 시간 
    {
        ...
        options: ["AWS Global Accelerator를 사용한다.", "Amazon CloudFront를 사용한다.", "Route 53 지연 시간 기반 라우팅을 사용한다.", "Application Load Balancer를 각 지역에 배포한다."],
    ...
    // Q14: S3 업로드 속도 개선 
    {
        ...
        options: ["S3 Transfer Acceleration을 활성화한다.", "S3 멀티파트 업로드를 비활성화한다.", "AWS Global Accelerator를 S3 앞에 둔다.", "각 지역별로 S3 버킷을 만들고 나중에 병합한다."],
    ...
    // Q15: HPC 고성능 스토리지 
    {
        ...
        options: ["Amazon FSx for Lustre", "Amazon EFS General Purpose", "Amazon EBS Provisioned IOPS", "Amazon S3 Glacier"],
    ...
    // Q16: 프라이빗 서브넷 인터넷 접속 
    {
        ...
        options: ["퍼블릭 서브넷에 NAT 게이트웨이를 배치하고 라우팅 테이블을 수정한다.", "프라이빗 서브넷에 인터넷 게이트웨이를 연결한다.", "데이터베이스에 퍼블릭 IP를 할당한다.", "NAT 인스턴스를 프라이빗 서브넷에 배치한다."],
    ...
    // Q17: EBS 스냅샷 실수 삭제 방지 
    {
        ...
        options: ["EBS 스냅샷용 휴지통(Recycle Bin) 규칙을 설정한다.", "S3 버킷 버전을 활성화한다.", "IAM 정책으로 DeleteSnapshot 권한을 모두 제거한다.", "스냅샷을 매일 다른 리전으로 복사한다."],
    ...
    // Q18: 데이터 암호화 및 처리 (Glue) 
    {
        ...
        options: ["AWS Glue", "Amazon EMR", "Amazon EC2에 Spark 설치", "Amazon Redshift"],
        answer: "AWS Glue",
    ...
    // Q19: 루트 계정 보안 
    {
        ...
        options: ["루트 사용자에게 MFA(다중 요소 인증)를 활성화한다.", "루트 사용자의 액세스 키를 생성하여 개발자에게 공유한다.", "일상적인 작업에 루트 사용자를 사용한다.", "루트 사용자의 비밀번호를 팀원들과 공유한다."],
    ...
    // Q20: EBS 암호화 강제 (SCP) 
    {
        ...
        options: ["AWS Organizations에서 SCP(서비스 제어 정책)를 사용하여 암호화되지 않은 볼륨 생성을 거부한다.", "모든 사용자의 IAM 정책을 수동으로 수정한다.", "CloudTrail 로그를 감시하여 위반 시 알림을 보낸다.", "각 리전별로 Config 규칙을 생성한다."],
    ...
    // Q21: S3 특정 폴더 접근 제어 
    {
        ...
        options: ["arn:aws:s3:::bucket-name/reports/*", "arn:aws:s3:::bucket-name", "arn:aws:s3:::bucket-name/*", "arn:aws:s3:::*"],
    ...
    // Q22: DynamoDB 데이터 일관성 
    {
        ...
        options: ["강력한 일관된 읽기(Strongly Consistent Read)를 요청한다.", "DynamoDB Accelerator(DAX)를 사용한다.", "테이블에 인덱스를 추가한다.", "읽기 용량을 늘린다."],
    ...
    // Q23: Aurora 읽기 성능 확장 
    {
        ...
        options: ["Aurora 읽기 전용 복제본(Read Replica)을 추가한다.", "인스턴스 타입을 더 큰 것으로 변경한다.", "데이터베이스를 DynamoDB로 마이그레이션한다.", "ElastiCache를 도입한다."],
    ...
    // Q24: 하이브리드 컨테이너 운영 
    {
        ...
        options: ["Amazon ECS Anywhere", "AWS Fargate", "Amazon EC2 Auto Scaling", "AWS Lambda"],
    ...
    // Q25: 로컬 데이터 처리 및 규정 준수 (Outposts) 
    {
        ...
        options: ["AWS Outposts를 데이터 센터에 설치하고 EKS를 실행한다.", "AWS Direct Connect를 연결한다.", "Snowball Edge 장비를 사용한다.", "로컬 PC에 Minikube를 설치한다."],
    ...
    // Q26: 고가용성 데이터베이스 아키텍처 
    {
        ...
        options: ["Amazon Aurora", "Amazon RDS for MariaDB", "Amazon EC2에 설치된 MySQL", "Amazon DynamoDB"],
    ...
    // Q27: 자연어 처리 아키텍처 
    {
        ...
        options: ["API Gateway + Lambda + Amazon Comprehend", "EC2 + Textract", "Kinesis + SageMaker", "S3 + Athena"],
    ...
    // Q28: 소규모 데이터 전송 
    {
        ...
        options: ["AWS Site-to-Site VPN", "AWS Direct Connect", "인터넷 게이트웨이", "VPC 피어링"],
    ...
    // Q29: 비동기 작업 처리 
    {
        ...
        options: ["Amazon SQS와 Auto Scaling 그룹", "Amazon SNS와 Lambda", "Kinesis Data Streams", "DynamoDB Streams"],
    ...
    // Q30: 게임 점수판 (Leaderboard) 
    {
        ...
        options: ["Amazon ElastiCache for Redis", "Amazon DynamoDB", "Amazon RDS", "Amazon S3"],
        answer: "Amazon ElastiCache for Redis",
    ...
    // Q31: 암호화 키 관리 
    {
        ...
        options: ["AWS KMS의 고객 관리형 키(CMK)를 고객별로 생성하여 사용한다.", "S3 관리형 키(SSE-S3)를 사용한다.", "클라이언트가 직접 키를 생성하여 관리하게 한다.", "단일 KMS 키를 모든 고객에게 사용한다."],
    ...
    // Q32: 스트리밍 데이터 수집 
    {
        ...
        options: ["Amazon Kinesis Data Firehose", "Amazon Kinesis Data Streams", "AWS Lambda", "Amazon EC2"],
    ...
    // Q33: S3 보안 강화 
    {
        ...
        options: ["S3 일괄 작업(Batch Operations)으로 기존 객체를 암호화하고, 버킷 정책으로 암호화된 업로드만 허용한다.", "모든 객체를 다운로드 후 다시 업로드한다.", "AWS Config를 사용하여 규정 위반을 탐지한다.", "KMS 키를 삭제한다."],
    ...
    // Q34: KMS 키 자동 교체 
    {
        ...
        options: ["AWS KMS의 자동 키 교체 기능을 활성화한다.", "매년 수동으로 새 키를 만들고 애플리케이션을 업데이트한다.", "S3 관리형 키를 사용한다.", "CloudHSM을 사용한다."],
    ...
    // Q35: 인프라 상태 모니터링 
    {
        ...
        options: ["AWS Health Dashboard (Personal Health Dashboard)", "AWS CloudTrail", "Amazon CloudWatch Logs", "AWS Trusted Advisor"],
...
// --- 36번 ~ 65번 문제 추가 ---
    ...
    // Q36: 프라이빗 S3 접근 및 온프레미스 연동 [Q36]
    {
        ...
        options: ["S3용 게이트웨이 VPC 엔드포인트와 AWS Direct Connect를 사용한다.", "EC2용 인터페이스 VPC 엔드포인트를 사용한다.", "NAT 게이트웨이를 통해 프록시 서버를 구성한다.", "Transit Gateway와 VPN을 사용한다."],
    ...
    // Q37: 리눅스 애플리케이션 공유 스토리지 [Q37]
    {
        ...
        options: ["Amazon EFS (Elastic File System)", "Amazon S3 Standard", "Amazon EBS (Elastic Block Store)", "AWS Storage Gateway"],
    ...
    // Q38: 서버리스 마이크로서비스 아키텍처 [Q38]
    {
        category: "서버리스",
        title: "관리 오버헤드를 최소화하면서 확장 가능한 마이크로서비스 API를 구축하려고 합니다. 트래픽 급증을 처리할 수 있고 유휴 상태일 때는 비용이 발생하지 않는 구조는 무엇입니까?",
        options: ["Amazon API Gateway + AWS Lambda", "Application Load Balancer + Amazon EC2", "Amazon ECS + Fargate", "AWS Elastic Beanstalk"],
    ...
    // Q39: 읽기 집약적 데이터베이스 부하 분산 [Q39]
    {
        category: "데이터베이스",
        title: "Amazon RDS를 사용하는 웹 애플리케이션에서 읽기 트래픽이 폭주하여 데이터베이스 성능이 저하되고 있습니다. 애플리케이션 응답 속도를 개선하기 위해 가장 효과적인 캐싱 전략은 무엇입니까?",
        options: ["Amazon ElastiCache for Redis를 도입하여 쿼리 결과를 캐싱한다.", "RDS 인스턴스 크기를 두 배로 늘린다.", "RDS 다중 AZ(Multi-AZ) 배포를 활성화한다.", "DynamoDB로 데이터베이스를 마이그레이션한다."],
    ...
    // Q40: 데이터 레이크 보안 (Row-level Security) [Q40]
    {
        category: "분석",
        title: "AWS Lake Formation을 사용하여 데이터 레이크를 구축했습니다. 특정 부서가 데이터의 민감한 열(Column)이나 행(Row)을 보지 못하도록 제한하려고 합니다. 가장 효율적인 방법은 무엇입니까?",
        options: ["Lake Formation의 데이터 필터(Data Filters) 기능을 사용한다.", "데이터를 별도의 S3 버킷으로 복사하여 권한을 분리한다.", "IAM 정책을 사용하여 특정 S3 객체에 대한 접근을 차단한다.", "Lambda 함수로 데이터를 마스킹하여 제공한다."],
    ...
    // Q41: 예측 가능한 트래픽 스파이크 대응 [Q41]
    {
        category: "컴퓨팅",
        title: "매일 아침 9시에 트래픽이 급증하는 업무용 애플리케이션이 있습니다. 사용자 경험을 저해하지 않으면서 미리 용량을 확보하려면 Auto Scaling 그룹을 어떻게 설정해야 합니까?",
        options: ["예약된 조정(Scheduled Scaling) 작업을 설정한다.", "대상 추적(Target Tracking) 조정 정책을 사용한다.", "단계 조정(Step Scaling) 정책을 사용한다.", "항상 최대 용량으로 인스턴스를 유지한다."],
    ...
    // Q43: DDoS 공격 방어 [Q43]
    {
        ...
        options: ["AWS WAF를 ALB에 연결하고 속도 기반 규칙(Rate-based rule)을 설정한다.", "ALB의 보안 그룹에서 해당 IP를 차단한다.", "Amazon GuardDuty를 활성화하여 공격을 탐지한다.", "CloudFront를 배포하고 모든 트래픽을 캐싱한다."],
    ...
    // Q46: 보안 그룹 포트 감사 [Q46]
    {
        ...
        options: ["AWS Config 관리형 규칙(restricted-ssh)을 사용하고 위반 시 SNS로 알림을 보낸다.", "Lambda 함수를 작성하여 매시간 보안 그룹을 검사한다.", "VPC 흐름 로그를 분석하여 SSH 트래픽을 감지한다.", "CloudTrail 로그를 Athena로 쿼리한다."],
        answer: "AWS Config 관리형 규칙(restricted-ssh)을 사용하고 위반 시 SNS로 알림을 보낸다.",
        explanation: "AWS Config는 'restricted-ssh'와 같은 사전 정의된 관리형 규칙을 제공하여, 별도의 코드 작성 없이 보안 그룹 위반 사항을 감지하고 알림을 보낼 수 있습니다."
    },

    // Q47: S3 비용 효율적 접근 [Q47]
    {
        category: "네트워크",
        title: "프라이빗 서브넷의 EC2 인스턴스가 S3 버킷의 대용량 데이터를 자주 다운로드해야 합니다. NAT 게이트웨이 처리 비용을 절감하면서 S3에 접근하는 가장 좋은 방법은 무엇입니까?",
        options: ["S3용 게이트웨이 VPC 엔드포인트(Gateway VPC Endpoint)를 생성한다.", "NAT 게이트웨이를 하나 더 추가한다.", "EC2 인스턴스에 퍼블릭 IP를 할당한다.", "AWS Direct Connect를 사용한다."],
    ...
    // Q48: 서버리스 데이터 처리 파이프라인 [Q48]
    {
        category: "분석",
        title: "S3에 업로드된 데이터를 정제하고 변환하는 작업을 코드를 작성하지 않고 시각적으로 구성하고 싶습니다. 또한 이 작업은 이벤트에 따라 자동으로 실행되어야 합니다. 적절한 서비스 조합은?",
        options: ["AWS Glue DataBrew + AWS Step Functions", "AWS Lambda + Amazon SNS", "Amazon EMR + Apache Spark", "Amazon Athena + QuickSight"],
    ...
    // Q49: 배치 작업 처리 아키텍처 [Q49]
    {
        category: "아키텍처",
        title: "이미지 처리 작업을 수행하는 배치 애플리케이션이 있습니다. 작업 요청이 불규칙적으로 들어오며, 처리되지 않은 작업은 손실되지 않고 나중에라도 반드시 처리되어야 합니다. 가장 적합한 아키텍처는?",
        options: ["Amazon SQS + EC2 Auto Scaling 그룹", "Amazon SNS + Lambda", "Kinesis Data Streams + EMR", "EC2 인스턴스에 Cron 작업 설정"],
    ...
    // Q50: 이기종 데이터베이스 마이그레이션 [Q50]
    {
        category: "데이터베이스",
        title: "온프레미스 Oracle 데이터베이스를 AWS Aurora PostgreSQL로 마이그레이션하려고 합니다. 스키마 변환과 데이터 복제를 모두 수행해야 합니다. 어떤 도구 조합을 사용해야 합니까?",
        options: ["AWS SCT(Schema Conversion Tool) + AWS DMS(Database Migration Service)", "AWS DataSync + S3", "Oracle Data Pump + Aurora Read Replica", "Native PostgreSQL Backup/Restore"],
    ...
    // Q53: 정적 웹사이트 호스팅 [Q53]
    {
        ...
        options: ["Amazon S3 정적 웹사이트 호스팅 + Amazon CloudFront", "EC2 인스턴스에 Apache 웹서버 설치", "AWS Elastic Beanstalk 사용", "Lambda 함수로 HTML 반환"],
    ...
    // Q54: 대규모 배치 처리 [Q54]
    {
        ...
        options: ["AWS Batch", "Amazon ECS", "AWS Lambda", "EC2 스팟 인스턴스 직접 관리"],
        answer: "AWS Batch",
    ...
    // Q55: 리전 간 백업 (DR) [Q55]
    {
        ...
        options: ["AWS Backup을 사용하여 리전 간 복사(Cross-Region Copy)를 구성한다.", "Lambda 함수로 스냅샷을 찍고 S3로 복사하는 스크립트를 짠다.", "RDS의 읽기 전용 복제본을 다른 리전에 생성한다.", "S3 CRR(Cross-Region Replication)을 사용한다."],
    ...
    // Q56: 트래픽 폭주 시 데이터베이스 보호 [Q56]
    {
        category: "아키텍처",
        title: "마케팅 이벤트로 인해 트래픽이 폭주할 때, 데이터베이스에 쓰기 요청이 몰려 DB가 다운될 위험이 있습니다. 요청을 유실하지 않고 DB 부하를 조절(Throttling)하려면 어떤 구조를 도입해야 합니까?",
        options: ["Amazon SQS를 DB 앞단에 두어 요청을 버퍼링한다.", "RDS 인스턴스 타입을 최대 크기로 늘린다.", "DynamoDB로 교체한다.", "API Gateway의 스로틀링 기능을 사용한다."],
    ...
    // Q57: 실시간 스트리밍 데이터 분석 [Q57]
    {
        category: "분석",
        title: "웹사이트 클릭스트림 데이터를 실시간으로 수집하여 분석 대시보드에 표시하려고 합니다. 데이터 변환 후 S3 데이터 레이크에도 저장해야 합니다. 가장 적합한 서비스 조합은?",
        options: ["Amazon Kinesis Data Firehose + Amazon Redshift/S3", "SQS + Lambda + RDS", "EC2 + Kafka + EBS", "Glue + Athena"],
    ...
    // Q58: 하이브리드 파일 스토리지 (SMB) [Q58]
    {
        ...
        options: ["AWS Storage Gateway (File Gateway)", "AWS DataSync", "Amazon FSx for Windows File Server", "S3 Direct Put"],
    ...
    // Q59: DynamoDB 변경 사항 트리거 [Q59]
    {
        category: "데이터베이스",
        title: "DynamoDB 테이블의 데이터가 변경될 때마다 즉시 다운스트림 서비스에 알림을 보내야 합니다. 이를 구현하는 가장 효율적인 방법은 무엇입니까?",
        options: ["DynamoDB Streams를 활성화하고 Lambda를 트리거하여 SNS로 알림을 보낸다.", "애플리케이션에서 업데이트 후 SNS를 호출한다.", "매분 테이블을 스캔하는 Lambda를 만든다.", "CloudWatch Logs를 구독한다."],
    ...
    // Q62: 글로벌 사용자를 위한 콘텐츠 전송 [Q62]
    {
        category: "네트워크",
        title: "전 세계 사용자에게 정적 및 동적 콘텐츠를 제공하는 웹 애플리케이션이 있습니다. 사용자의 언어 설정(Accept-Language 헤더)에 따라 다른 콘텐츠를 캐싱하여 제공하려면 어떻게 해야 합니까?",
        options: ["CloudFront 배포를 생성하고 Accept-Language 헤더를 화이트리스트에 추가하여 캐싱한다.", "S3 버킷을 지역별로 복제한다.", "Route 53 지리적 위치 라우팅을 사용한다.", "ALB에서 언어를 감지하여 리디렉션한다."],
    ...
    // Q63: 데이터베이스 로그인 감시 [Q63]
    {
        category: "보안",
        title: "Amazon Aurora 데이터베이스에 대한 모든 로그인 시도(성공 및 실패)를 감사하고 싶습니다. 로그 데이터는 분석을 위해 중앙의 S3 버킷으로 모아야 합니다. 가장 효율적인 방법은?",
        options: ["Aurora 감사 로그(Audit Logs)를 활성화하고 CloudWatch Logs로 보낸 뒤, 구독 필터를 통해 S3로 내보낸다.", "CloudTrail을 사용하여 데이터베이스 로그인을 추적한다.", "애플리케이션에서 로그인 로그를 직접 S3로 쏜다.", "VPC 흐름 로그를 분석한다."],
    ...
    // Q64: S3 악성 활동 탐지 [Q64]
    {
        ...
        options: ["Amazon GuardDuty", "Amazon Macie", "AWS WAF", "AWS Shield"],
    ...
    // Q65: 멀티 계정 환경 구축 [Q65]
    {
        ...
        options: ["AWS Control Tower", "AWS Organizations 만 단독 사용", "AWS Service Catalog", "Terraform 스크립트"],
...
// --- 66번 ~ 105번 문제 추가 ---
    ...
    // Q66: 비용 관리 및 예측 [Q66]
    {
        ...
        options: ["AWS Cost Explorer API를 사용한다.", "AWS Budgets 보고서를 이메일로 받아 파싱한다.", "Cost & Usage Report(CUR)를 S3에 저장하고 Athena로 쿼리한다.", "AWS Billing 콘솔에서 CSV를 수동으로 다운로드한다."],
    ...
    // Q67: S3 스토리지 계층 최적화 [Q67]
    {
        category: "저장소",
        title: "수 페타바이트의 데이터가 S3 Standard에 저장되어 있습니다. 데이터 접근 패턴이 불규칙하여 자주 액세스되는지 알 수 없습니다. 성능 저하 없이 비용을 최적화하려면 어떤 스토리지 클래스를 사용해야 합니까?",
        options: ["S3 Intelligent-Tiering", "S3 Standard-Infrequent Access (S3 Standard-IA)", "S3 Glacier Instant Retrieval", "S3 One Zone-IA"],
    ...
    // Q68: EBS 다중 연결 (Multi-Attach) [Q68]
    {
        category: "컴퓨팅",
        title: "단일 EBS 볼륨을 여러 개의 EC2 Nitro 인스턴스에 동시에 연결하여 쓰기 작업을 수행해야 합니다. 고가용성을 위해 어떤 EBS 볼륨 유형을 사용해야 합니까?",
        options: ["Provisioned IOPS SSD (io2)", "General Purpose SSD (gp3)", "Throughput Optimized HDD (st1)", "Cold HDD (sc1)"],
    ...
    // Q69: 정적 웹사이트 보안 (CloudFront & WAF) [Q69]
    {
        category: "보안",
        title: "S3 버킷에서 호스팅되는 정적 웹사이트가 있습니다. 특정 사내 IP 주소에서만 접근을 허용하고, HTTPS를 강제하며, S3 직접 접근을 차단하고 싶습니다. 어떤 구성이 필요합니까?",
        options: ["CloudFront를 생성하고 OAC(Origin Access Control)를 설정한 뒤, AWS WAF를 CloudFront에 연결하여 IP를 제한한다.", "S3 버킷 정책에서 IP 제한을 설정하고 정적 웹 호스팅 기능을 켠다.", "S3 버킷을 프라이빗 서브넷으로 옮긴다.", "Route 53에서 지리적 차단을 설정한다."],
    ...
    // Q70: EBS 암호화 강제 (계정 설정) [Q70]
    {
        category: "보안",
        title: "규정 준수를 위해 EC2 인스턴스에 연결되는 모든 새로운 EBS 볼륨이 생성될 때 자동으로 암호화되도록 보장하고 싶습니다. 가장 관리 부담이 적은 방법은 무엇입니까?",
        options: ["EC2 대시보드의 'EBS 암호화(Encryption by default)' 계정 설정을 활성화한다.", "IAM 정책을 사용하여 암호화되지 않은 볼륨 생성을 거부한다.", "AWS Config 규칙을 사용하여 암호화되지 않은 볼륨을 탐지하고 삭제한다.", "KMS 키 정책을 수정하여 모든 사용자가 암호화를 사용하도록 강제한다."],
        answer: "EC2 대시보드의 'EBS 암호화(Encryption by default)' 계정 설정을 활성화한다.",
        explanation: "리전별로 '기본적으로 암호화(Encryption by default)' 설정을 켜두면, 해당 리전에서 생성되는 모든 새 EBS 볼륨이 자동으로 암호화되므로 별도의 정책 관리가 필요 없습니다."
    },

    // Q71: 글로벌 데이터베이스 (DynamoDB Global Tables) [Q71]
    {
        category: "데이터베이스",
        ...
        options: ["Amazon DynamoDB Global Tables", "Amazon Aurora Global Database", "Amazon RDS for MySQL with Read Replicas", "Amazon ElastiCache Global Datastore"],
    ...
    // Q72: 프라이빗 DynamoDB 접근 [Q72]
    {
        category: "네트워크",
        title: "프라이빗 서브넷의 EC2 인스턴스에서 DynamoDB에 접근하려고 합니다. 데이터가 공용 인터넷을 통과하지 않도록 하려면 무엇을 설정해야 합니까?",
        options: ["DynamoDB용 게이트웨이 VPC 엔드포인트(Gateway VPC Endpoint)", "DynamoDB용 인터페이스 VPC 엔드포인트", "NAT 게이트웨이", "VPN 연결"],
        answer: "DynamoDB용 게이트웨이 VPC 엔드포인트(Gateway VPC Endpoint)",
        explanation: "S3와 DynamoDB는 '게이트웨이' 유형의 VPC 엔드포인트를 사용합니다. 이를 통해 라우팅 테이블에서 트래픽을 처리하며 인터넷 없이 AWS 네트워크 내부에서 접근 가능합니다."
    },

    // Q73: 재해 복구 정적 페이지 (Failover Routing) [Q73]
    {
        category: "네트워크",
        title: "웹 애플리케이션의 기본 리전이 다운되었을 때, 사용자를 S3 버킷에 호스팅된 정적 오류 페이지(점검 중 화면)로 리디렉션하고 싶습니다. Route 53 라우팅 정책은 무엇입니까?",
        options: ["장애 조치(Failover) 라우팅 정책", "지연 시간(Latency) 라우팅 정책", "가중치 기반(Weighted) 라우팅 정책", "지리적 위치(Geolocation) 라우팅 정책"],
    ...
    // Q74: 예약 인스턴스 구매 전략 [Q74]
    {
        category: "비용 최적화",
        title: "향후 3년 동안 24시간 내내 실행되어야 하는 데이터베이스 서버가 있습니다. 예산이 한정되어 있어 초기 비용(Upfront)을 낼 수 없을 때, 가장 비용 효율적인 예약 인스턴스(RI) 옵션은 무엇입니까?",
        options: ["No Upfront (선결제 없음) 예약 인스턴스", "All Upfront (전체 선결제) 예약 인스턴스", "Partial Upfront (부분 선결제) 예약 인스턴스", "온디맨드 인스턴스"],
        answer: "No Upfront (선결제 없음) 예약 인스턴스",
        explanation: "초기 비용을 지불할 수 없는 경우 'No Upfront' 옵션을 선택하면 약정 기간 동안 월별로 할인된 요금을 지불할 수 있어 온디맨드보다 저렴합니다."
    },

    // Q75: 윈도우 파일 서버 공유 [Q75]
    {
        category: "저장소",
        title: "여러 가용 영역(AZ)에 걸쳐 있는 Windows EC2 인스턴스들이 공통의 Windows 파일 시스템(SMB)을 공유해야 합니다. 액티브 디렉터리(AD)와 통합되어야 합니다. 어떤 서비스가 적합합니까?",
        options: ["Amazon FSx for Windows File Server", "Amazon EFS", "Amazon EBS Multi-Attach", "S3 마운트"],
    ...
    // Q76: 실시간 순위표 (Leaderboard) [Q76]
    {
        category: "데이터베이스",
        title: "온라인 게임에서 상위 10명의 점수를 실시간으로 보여주는 순위표(Leaderboard) 기능이 필요합니다. 지연 시간이 짧고 구현이 쉬운 서비스는?",
        options: ["Amazon ElastiCache for Redis", "Amazon RDS for MySQL", "Amazon DynamoDB", "Amazon Redshift"],
        answer: "Amazon ElastiCache for Redis",
    ...
    // Q77: KMS 키 자동 교체 [Q77]
    {
        ...
        options: ["AWS KMS 고객 관리형 키(CMK)를 생성하고 자동 교체를 활성화한다.", "S3 관리형 키(SSE-S3)를 사용한다.", "매년 수동으로 새 키를 만들고 데이터를 재암호화한다.", "클라이언트 측 암호화를 사용한다."],
    ...
    // Q78: S3 업로드 가속화 [Q78]
    {
        ...
        options: ["S3 Transfer Acceleration을 활성화한다.", "CloudFront 배포를 생성하고 S3를 오리진으로 설정한 뒤 PUT 메서드를 허용한다.", "Global Accelerator를 사용한다.", "S3 멀티파트 업로드만 사용한다."],
        answer: "S3 Transfer Acceleration을 활성화한다.",
    ...
    // Q79: 중앙 집중식 계정 관리 [Q79]
    {
        category: "관리 및 거버넌스",
        title: "여러 부서의 AWS 계정을 통합 관리하고, 직원들이 기존 사내 Active Directory 자격 증명으로 모든 계정에 로그인할 수 있게 하려 합니다. 가장 적합한 서비스 조합은?",
        options: ["AWS Organizations 및 AWS IAM Identity Center (구 SSO)", "IAM 사용자 생성 및 Cross-Account 역할", "Cognito User Pools", "Directory Service 만 사용"],
    ...
    // Q80: 데이터 전송 (DataSync) [Q80]
    {
        category: "마이그레이션",
        title: "온프레미스 스토리지의 10KB 크기 작은 파일 수백만 개를 S3로 이전해야 합니다. 네트워크 대역폭을 효율적으로 사용하고 전송 속도를 최적화하려면 어떤 서비스를 사용해야 합니까?",
        options: ["AWS DataSync", "AWS Snowball Edge", "S3 CLI (aws s3 cp)", "AWS Transfer Family"],
        answer: "AWS DataSync",
        explanation: "AWS DataSync는 네트워크 프로토콜을 최적화하여 작은 파일이 많은 경우에도 인터넷이나 Direct Connect를 통해 빠르고 효율적으로 데이터를 전송합니다."
    },

    // Q81: 중복 없는 메시지 처리 (SQS FIFO) [Q81]
    {
        category: "애플리케이션 통합",
        title: "금융 거래 신청서를 처리하는 시스템에서 각 신청서가 정확히 한 번만, 순서대로 처리되어야 합니다. 어떤 메시징 서비스를 사용해야 합니까?",
        options: ["Amazon SQS FIFO 대기열", "Amazon SQS 표준 대기열", "Amazon SNS", "Amazon Kinesis Data Streams"],
        answer: "Amazon SQS FIFO 대기열",
        explanation: "SQS FIFO(선입선출) 대기열은 메시지의 순서를 보장하고, '정확히 1회 처리(Exactly-Once Processing)'를 지원하여 중복을 방지합니다."
    },

    // Q82: 실시간 데이터 스트리밍 분석 [Q82]
    {
        category: "분석",
        title: "다양한 소스에서 발생하는 로그 데이터를 실시간으로 수집하고, SQL을 사용하여 실시간으로 분석한 뒤 결과를 S3에 저장하고 싶습니다. 어떤 아키텍처가 적합합니까?",
        options: ["Kinesis Data Streams 수집 -> Kinesis Data Analytics 분석 -> Kinesis Data Firehose 저장", "S3 저장 -> Athena 분석", "SQS 수집 -> EC2 분석 -> RDS 저장", "CloudWatch Logs 수집 -> Lambda 분석"],
    ...
    // Q85: VPC 엔드포인트 보안 [Q85]
    {
        category: "보안",
        title: "프라이빗 서브넷의 EC2 인스턴스가 S3에 접근할 때, 오직 특정 S3 버킷에만 접근할 수 있도록 제한하고 싶습니다. 이를 위한 가장 효과적인 방법은?",
        options: ["S3 게이트웨이 엔드포인트를 생성하고, 엔드포인트 정책(Endpoint Policy)에 특정 버킷만 허용하도록 명시한다.", "EC2 보안 그룹에서 아웃바운드 규칙을 제한한다.", "NACL을 사용하여 트래픽을 차단한다.", "IAM 사용자 정책을 수정한다."],
    ...
    // Q86: 컨테이너 이미지 스케줄링 [Q86]
    {
        category: "서버리스",
        title: "10분마다 실행되는 간단한 이미지 처리 작업이 있습니다. 이 작업은 Docker 컨테이너로 패키징되어 있습니다. 서버 관리 없이 예약된 시간에 실행하려면 어떤 서비스가 최적입니까?",
        options: ["AWS Lambda (컨테이너 이미지 지원) + EventBridge Scheduler", "Amazon ECS Fargate + Scheduled Task", "EC2 인스턴스 + Crontab", "AWS Batch"],
    ...
    // Q87: S3 접근 비용 절감 [Q87]
    {
        ...
        options: ["S3용 게이트웨이 VPC 엔드포인트를 설정하여 라우팅 테이블을 업데이트한다.", "NAT 게이트웨이를 더 큰 사이즈로 변경한다.", "EC2를 퍼블릭 서브넷으로 옮긴다.", "S3 Transfer Acceleration을 사용한다."],
    ...
    // Q88: EKS 노드 비용 절감 [Q88]
    {
        category: "컨테이너",
        title: "Amazon EKS 클러스터에서 중단되어도 상관없는 배치 분석 작업을 실행 중입니다. 워커 노드(Worker Node) 비용을 최대 90%까지 절감하려면 어떤 전략을 써야 합니까?",
        options: ["관리형 노드 그룹(Managed Node Group)에서 스팟 인스턴스를 사용한다.", "온디맨드 인스턴스를 사용한다.", "Fargate를 사용한다.", "전용 호스트를 사용한다."],
    ...
    // Q89: 웹/DB 계층 확장성 [Q89]
    {
        ...
        options: ["ALB + EC2 Auto Scaling (3 AZ) + Aurora MySQL (Multi-AZ 및 Read Replica)", "NLB + EC2 + DynamoDB", "EC2 단일 인스턴스 + RDS Multi-AZ", "Route 53 + S3 정적 호스팅"],
    ...
    // Q90: DB 고성능 스토리지 (IOPS) [Q90]
    {
        category: "데이터베이스",
        title: "I/O가 매우 빈번한(I/O Intensive) 미션 크리티컬 데이터베이스를 RDS에서 운영 중입니다. 지연 시간을 최소화하고 일관된 IOPS 성능을 보장하려면 어떤 스토리지 유형을 선택해야 합니까?",
        options: ["Provisioned IOPS SSD (io1/io2)", "General Purpose SSD (gp3)", "Magnetic (Standard)", "Throughput Optimized HDD (st1)"],
    ...
    // Q91: Oracle DB 라이선스 유지 (RDS Custom) [Q91]
    {
        category: "데이터베이스",
        title: "기존에 보유한 Oracle 라이선스(BYOL)를 사용해야 하며, 데이터베이스 운영체제(OS)에 대한 접근 권한과 커스터마이징이 필요합니다. AWS에서 이를 지원하는 관리형 서비스는?",
        options: ["Amazon RDS Custom for Oracle", "Amazon RDS for Oracle", "EC2에 Oracle 설치", "Amazon Aurora"],
    ...
    // Q92: 대규모 데이터 오프라인 전송 [Q92]
    {
        category: "마이그레이션",
        title: "인터넷 대역폭이 제한적인 환경에서 600TB의 데이터를 2주 안에 AWS로 옮겨야 합니다. 가장 빠르고 비용 효율적인 방법은?",
        options: ["AWS Snowball Edge Storage Optimized 장비 여러 대 사용", "AWS Direct Connect 10Gbps 설치", "Site-to-Site VPN 사용", "S3 Multipart Upload"],
    ...
    // Q93: 테이프 백업 대체 [Q93]
    {
        ...
        options: ["AWS Storage Gateway (Tape Gateway)", "AWS DataSync", "Amazon S3 Glacier", "EBS Snapshots"],
    ...
    // Q96: 프라이빗 연결 (PrivateLink) [Q96]
    {
        category: "네트워크",
        title: "서로 다른 AWS 계정의 VPC에 있는 애플리케이션들이 인터넷을 거치지 않고 서로 통신해야 합니다. VPC 피어링을 사용하지 않고 '서비스 제공자 - 소비자' 모델로 연결하려면 무엇을 써야 합니까?",
        options: ["AWS PrivateLink (VPC Endpoint Service)", "VPC Peering", "Transit Gateway", "VPN Connection"],
    ...
    // Q99: 정적 콘텐츠 캐싱 [Q99]
    {
        ...
        options: ["Amazon CloudFront 배포를 생성하여 정적 콘텐츠를 캐싱한다.", "S3 버킷을 웹 서버로 사용한다.", "ElastiCache를 도입한다.", "EC2 인스턴스 유형을 업그레이드한다."],
    ...
    // Q100: S3 비용 절감 (Intelligent-Tiering) [Q100]
    {
        category: "비용 최적화",
        title: "S3에 저장된 데이터의 액세스 패턴이 매우 불규칙합니다. 어떤 파일은 자주 쓰이고, 어떤 파일은 거의 안 쓰입니다. 관리 노력 없이 비용을 최적화하려면?",
        options: ["S3 Intelligent-Tiering 스토리지 클래스를 사용한다.", "S3 Standard-IA로 모두 옮긴다.", "수명 주기 정책으로 30일 뒤 Glacier로 보낸다.", "S3 One Zone-IA를 사용한다."],
    ...
    // Q101: 3계층 웹 아키텍처 [Q101]
    {
        category: "아키텍처",
        title: "프론트엔드(React), 백엔드(Node.js), 데이터베이스로 구성된 3계층 애플리케이션을 구축합니다. 프론트엔드는 정적 호스팅을 하고 백엔드는 API로 호출하려 합니다. 가장 비용 효율적인 구조는?",
        options: ["프론트엔드: S3 + CloudFront, 백엔드: API Gateway + Lambda + DynamoDB", "프론트엔드: EC2, 백엔드: EC2 + RDS", "모두 Elastic Beanstalk에 배포", "Lightsail 인스턴스 사용"],
    ...
    // Q102: 미디어 트랜스코딩 [Q102]
    {
        ...
        options: ["AWS Elemental MediaConvert", "EC2에 FFmpeg 설치", "AWS Lambda만 사용", "Kinesis Video Streams"],
    ...
    // Q103: NACL vs 보안 그룹 [Q103]
    {
        category: "보안",
        title: "특정 악성 IP 주소 하나에서 오는 트래픽을 서브넷 수준에서 완전히 차단하고 싶습니다. 무엇을 사용해야 합니까?",
        options: ["네트워크 ACL (NACL)에 거부(Deny) 규칙 추가", "보안 그룹에 거부 규칙 추가", "Route 53에서 차단", "WAF에서 IP 차단 (서브넷 수준 아님)"],
    ...
    // Q104: MongoDB 호환 DB [Q104]
    {
        category: "데이터베이스",
        title: "온프레미스에서 사용 중인 MongoDB 워크로드를 AWS로 마이그레이션하려고 합니다. 애플리케이션 코드 변경을 최소화하면서 완전 관리형 서비스를 사용하고 싶습니다.",
        options: ["Amazon DocumentDB (with MongoDB compatibility)", "Amazon DynamoDB", "Amazon RDS for MySQL", "Amazon Neptune"],
...
// --- 106번 ~ 145번 문제 추가 ---
    ...
    // Q106: 의료 데이터 처리 및 개인정보 보호 
    {
        ...
        options: ["Amazon Textract + Amazon Comprehend Medical + Amazon Athena", "Amazon Rekognition + Amazon Transcribe Medical + RDS", "EC2 OCR 소프트웨어 + EMR", "Lambda + DynamoDB"],
    ...
    // Q107: 프라이빗 인스턴스 원격 접속 (Session Manager) 
    {
        ...
        options: ["AWS Systems Manager Session Manager를 사용한다.", "SSH 키를 사용하여 직접 접속한다.", "EC2 인스턴스 연결(EC2 Instance Connect)을 사용한다.", "RDP를 사용한다."],
    ...
    // Q108: 온프레미스 데이터 백업 (Storage Gateway) 
    {
        ...
        options: ["AWS Storage Gateway의 볼륨 게이트웨이(Cached Volumes) 모드", "AWS Storage Gateway의 볼륨 게이트웨이(Stored Volumes) 모드", "AWS Snowball Edge", "S3로 직접 업로드"],
    ...
    // Q109: Lambda 콜드 스타트 해결 
    {
        ...
        options: ["Lambda 함수에 프로비저닝된 동시성(Provisioned Concurrency)을 설정한다.", "Lambda 메모리를 최대치로 늘린다.", "API Gateway 앞에 CloudFront를 둔다.", "DynamoDB 읽기 용량을 늘린다."],
    ...
    // Q110: 컨테이너 오케스트레이션 선택 
    {
        ...
        options: ["Amazon ECS + AWS Fargate + Amazon RDS", "Amazon EKS + EC2 + DynamoDB", "EC2 직접 설치 + RDS", "Lambda + S3"],
    ...
    // Q111: HPC 네트워크 성능 최적화 
    {
        ...
        options: ["클러스터 배치 그룹(Cluster Placement Group)", "분산 배치 그룹(Spread Placement Group)", "파티션 배치 그룹(Partition Placement Group)", "기본 배치"],
    ...
    // Q112: 네트워크 대역폭 없는 대용량 데이터 이동 
    {
        ...
        options: ["AWS Snowball Edge Storage Optimized 장비로 데이터 이동 후, AWS Glue로 변환", "AWS DataSync 사용", "S3 Transfer Acceleration 사용", "Direct Connect 설치"],
    ...
    // Q113: 메시지 처리 실패 관리 (DLQ) 
    {
        ...
        options: ["SQS에 배달 못한 편지 대기열(Dead Letter Queue, DLQ)을 구성한다.", "Lambda 함수에서 오류를 무시하도록 코드 수정", "SNS 주제로 알림을 보낸다.", "CloudWatch Logs만 확인한다."],
    ...
    // Q114: 대용량 데이터베이스 마이그레이션 
    {
        ...
        options: ["AWS Snowball Edge로 초기 데이터를 옮기고, AWS DMS로 변경 사항을 복제한다.", "VPN을 통해 데이터를 전송한다.", "Direct Connect 10Gbps를 설치한다.", "DataSync를 사용한다."],
    ...
    // Q115: 윈도우 파일 서버 및 정적 웹 호스팅 
    {
        ...
        options: ["정적 파일: S3 + CloudFront, 공유 스토리지: Amazon FSx for Windows File Server", "모두 EBS 볼륨 사용", "S3와 EFS 사용", "EC2 인스턴스 스토어 사용"],
    ...
    // Q116: SFTP 서버 구축 
    {
        ...
        options: ["AWS Transfer Family", "EC2에 SFTP 서버 구축", "S3 버킷을 직접 공개", "Storage Gateway 사용"],
    ...
    // Q117: 오라클 데이터베이스 비용 최적화 
    {
        ...
        options: ["Amazon RDS for Oracle (License Included) 및 스토리지 자동 확장 활성화", "Aurora PostgreSQL로 마이그레이션", "EC2 인스턴스 타입 변경", "DynamoDB로 변경"],
    ...
    // Q118: API 접근 제어 (Cognito) 
    {
        ...
        options: ["Amazon Cognito User Pools와 API Gateway 권한 부여(Authorizer)", "IAM 사용자와 정책", "Lambda 함수 내에서 인증 로직 구현", "WAF로 IP 차단"],
    ...
    // Q119: S3 비용 절감 (Intelligent-Tiering) 
    {
        ...
        options: ["S3 Intelligent-Tiering 사용", "S3 Standard-IA로 이동", "S3 Glacier로 바로 이동", "S3 수명 주기 정책으로 매일 삭제"],
    ...
    // Q120: IAM 정책 권한 
    {
        ...
        options: ["최소 권한 원칙을 적용한 IAM 정책을 생성하여 사용자에게 연결한다.", "AdministratorAccess 정책을 연결한다.", "AWS Root 계정을 공유한다.", "보안 그룹을 수정한다."],
    ...
    // Q121: 리눅스 웹 서버 공유 스토리지 
    {
        ...
        options: ["Amazon EFS", "Amazon S3", "Amazon EBS", "Instance Store"],
        answer: "Amazon EFS",
    ...
    // Q122: 데이터 포맷 변환 (Glue) 
    {
        ...
        options: ["AWS Glue ETL 작업", "EMR 클러스터 생성", "EC2에서 스크립트 실행", "Lambda 함수로 하나씩 변환"],
    ...
    // Q123: 비동기 이벤트 처리 (EventBridge) 
    {
        ...
        options: ["Amazon EventBridge 사용", "SQS 대기열 하나 공유", "직접 HTTP 요청 호출", "데이터베이스 폴링"],
    ...
    // Q124: SQL 인젝션 방어 
    {
        ...
        options: ["AWS WAF (Web Application Firewall)", "Security Group", "Network ACL", "Amazon Inspector"],
        answer: "AWS WAF (Web Application Firewall)",
    ...
    // Q125: 크로스 계정 모니터링 
    {
        ...
        options: ["CloudWatch의 크로스 계정 관측성(Cross-Account Observability) 기능을 설정한다.", "모든 로그를 S3로 내보내서 분석한다.", "각 계정에 로그인하여 확인한다.", "타사 모니터링 도구를 설치한다."],
    ...
    // Q126: 로드 밸런서 SSL/TLS 인증서 
    {
        ...
        options: ["AWS Certificate Manager (ACM)", "IAM 인증서 저장소", "S3에 인증서 업로드", "외부 인증 기관에서 구매 후 수동 업로드"],
    ...
    // Q127: 정적 콘텐츠 성능 최적화 
    {
        ...
        options: ["이미지를 S3로 옮기고 CloudFront를 통해 제공한다.", "EC2 인스턴스 유형을 업그레이드한다.", "이미지 해상도를 낮춘다.", "데이터베이스에 이미지를 저장한다."],
    ...
    // Q128: HPC 데이터 스토리지 (FSx for Lustre) 
    {
        ...
        options: ["Amazon FSx for Lustre", "Amazon EFS", "Amazon EBS", "S3 Standard"],
        answer: "Amazon FSx for Lustre",
    ...
    // Q129: API Gateway 엔드포인트 유형 
    {
        ...
        options: ["엣지 최적화(Edge-optimized) 엔드포인트", "지역(Regional) 엔드포인트", "프라이빗(Private) 엔드포인트", "VPC 엔드포인트"],
    ...
    // Q130: 글로벌 사용자 API 지연 시간 감소 
    {
        ...
        options: ["API Gateway를 엣지 최적화(Edge-optimized)로 설정하고 캐싱을 활성화한다.", "Lambda 함수 메모리를 늘린다.", "RDS 읽기 전용 복제본을 만든다.", "리전별로 API를 따로 배포한다."],
    ...
    // Q131: 마이크로서비스 보안 (IAM 역할) 
    {
        ...
        options: ["각 마이크로서비스별로 IAM 역할을 생성하고 해당 인스턴스에 연결한다.", "하나의 강력한 IAM 역할을 모든 인스턴스에 공유한다.", "액세스 키를 코드에 하드코딩한다.", "IAM 사용자를 생성하여 자격 증명을 저장한다."],
    ...
    // Q132: 대용량 파일 전송 웹 앱 
    {
        ...
        options: ["S3 Transfer Acceleration", "S3 멀티파트 업로드", "Global Accelerator", "CloudFront PUT"],
    ...
    // Q133: 규정 준수 자동화 (Config & Systems Manager) 
    {
        ...
        options: ["AWS Config 규칙 및 AWS Systems Manager Automation", "CloudTrail 및 Lambda", "Inspector 및 GuardDuty", "Macie 및 Security Hub"],
    ...
    // Q134: 컨테이너 기반 작업 예약 
    {
        ...
        options: ["컨테이너 이미지를 사용하는 AWS Lambda와 EventBridge 스케줄러", "EC2 인스턴스와 윈도우 작업 스케줄러", "Fargate와 ECS 서비스", "AWS Batch"],
    ...
    // Q135: 데이터 웨어하우스와 머신러닝 
    {
        ...
        options: ["Amazon Redshift Serverless와 Redshift ML", "EMR과 Spark ML", "S3 데이터를 다운로드하여 로컬에서 학습", "SageMaker만 단독 사용"],
    ...
    // Q136: 온프레미스 스토리지 확장 (Storage Gateway) 
    {
        ...
        options: ["AWS Storage Gateway의 볼륨 게이트웨이 (Cached Volumes)", "S3 버킷 직접 마운트", "EFS 파일 시스템", "Snowball Edge"],
    ...
    // Q137: S3와 EFS 간 데이터 동기화 
    {
        ...
        options: ["AWS DataSync", "Lambda 함수 트리거", "EC2에서 cp 명령 실행", "S3 Replication"],
        answer: "AWS DataSync",
    ...
    // Q138: 개인정보(PII) 탐지 
    {
        ...
        options: ["Amazon Macie", "Amazon GuardDuty", "AWS Config", "AWS Inspector"],
        answer: "Amazon Macie",
    ...
    // Q139: S3 보안 접근 (Gateway Endpoint) 
    {
        ...
        options: ["S3용 게이트웨이 VPC 엔드포인트 생성", "NAT 게이트웨이 사용", "인터넷 게이트웨이 사용", "VPN 연결"],
    ...
    // Q141: 멀티 리전 보안 정책 (Control Tower) 
    {
        ...
        options: ["AWS Control Tower 및 AWS Organizations", "각 계정별 IAM 정책 수동 설정", "CloudFormation StackSets", "Config 규칙만 사용"],
    ...
    // Q142: 사진 저장소 비용 최적화 
    {
        ...
        options: ["S3 Intelligent-Tiering", "S3 Standard", "S3 Glacier Deep Archive", "DynamoDB"],
        answer: "S3 Intelligent-Tiering",
    ...
    // Q143: DB 부하 분산 (캐싱) 
    {
        ...
        options: ["Amazon ElastiCache를 도입하여 쿼리 결과를 캐싱한다.", "DB 인스턴스를 수직 확장한다.", "SNS를 사용하여 쿼리를 비동기 처리한다.", "Kinesis로 데이터를 스트리밍한다."],
    ...
    // Q144: 장기 데이터 보존 (AWS Backup) 
    {
        ...
        options: ["AWS Backup", "RDS 자동 백업 기능만 사용", "Lambda로 스냅샷 생성 스크립트 작성", "CloudWatch Logs에 데이터 저장"],
        answer: "AWS Backup",
    ...
    // Q145: 데이터 공유 (Lake Formation) 
    {
        ...
        options: ["AWS Lake Formation의 태그 기반 접근 제어", "S3 버킷 정책으로 공유", "데이터를 복제해서 전달", "IAM 역할 공유"],
...
// --- 146번 ~ 185번 문제 추가 ---
    ...
    // Q146: 주문 시스템 내결함성 (SQS) 
    {
        ...
        options: ["EC2 인스턴스를 Auto Scaling 그룹에 넣고, 주문을 Amazon SQS 대기열로 보내 처리한다.", "데이터베이스를 Multi-AZ로 변경한다.", "SNS를 사용하여 주문을 브로드캐스트한다.", "Lambda 함수를 사용하여 직접 DB에 쓴다."],
    ...
    // Q148: DynamoDB 용량 모드 선택 
    {
        ...
        options: ["온디맨드(On-demand) 용량 모드", "프로비저닝(Provisioned) 용량 모드", "Auto Scaling이 설정된 프로비저닝 모드", "DAX 사용"],
    ...
    // Q149: EC2 시작 속도 개선 (절전 모드) 
    {
        ...
        options: ["EC2 절전 모드(Hibernation)를 활성화하고 Auto Scaling 웜 풀(Warm Pool)을 구성한다.", "인스턴스 유형을 더 큰 것으로 변경한다.", "스팟 인스턴스를 사용한다.", "AMI를 매번 새로 굽는다."],
    ...
    // Q150: 정밀 모니터링 (CloudWatch) 
    {
        ...
        options: ["EC2 인스턴스에 대한 세부 모니터링(Detailed Monitoring)을 활성화한다.", "기본 모니터링(5분)을 그대로 사용한다.", "CloudWatch Logs 에이전트를 설치한다.", "VPC 흐름 로그를 켠다."],
    ...
    // Q151: 멀티 계정 보안 거버넌스 (Control Tower) 
    {
        ...
        options: ["AWS Control Tower", "AWS Config 만 단독 사용", "AWS Systems Manager", "IAM 정책 수동 관리"],
        answer: "AWS Control Tower",
    ...
    // Q152: 데이터베이스 연결 관리 (RDS Proxy) 
    {
        ...
        options: ["Amazon RDS Proxy를 사용한다.", "Lambda 동시성을 제한한다.", "RDS 인스턴스 크기를 늘린다.", "DynamoDB로 교체한다."],
    ...
    // Q153: 민감 데이터 자동 탐지 및 알림 
    {
        ...
        options: ["Amazon Macie로 탐지하고, EventBridge와 SNS를 연동하여 알림을 보낸다.", "Lambda 함수로 모든 파일을 스캔한다.", "GuardDuty를 사용하여 탐지한다.", "S3 액세스 로그를 분석한다."],
    ...
    // Q154: 비용 배분 태그 (Cost Allocation Tags) 
    {
        ...
        options: ["리소스에 태그(Tag)를 지정하고 '비용 배분 태그(Cost Allocation Tags)'를 활성화한 뒤 Cost Explorer에서 분석한다.", "계정을 애플리케이션별로 분리한다.", "AWS Budgets를 사용한다.", "CloudWatch 지표를 본다."],
        answer: "리소스에 태그(Tag)를 지정하고 '비용 배분 태그(Cost Allocation Tags)'를 활성화한 뒤 Cost Explorer에서 분석한다.",
    ...
    // Q155: 온프레미스 iSCSI 스토리지 확장 
    {
        ...
        options: ["AWS Storage Gateway 볼륨 게이트웨이(Stored Volumes)", "AWS Storage Gateway 볼륨 게이트웨이(Cached Volumes)", "S3 File Gateway", "Tape Gateway"],
    ...
    // Q156: 외부 ID 공급자 연동 (SAML 2.0) 
    {
        ...
        options: ["AWS IAM Identity Center (AWS SSO)를 사용하여 IdP와 연결한다.", "각 직원용 IAM 사용자를 만든다.", "Cognito를 사용한다.", "AD Connector를 사용한다."],
    ...
    // Q157: 크로스 계정 관측성 
    {
        ...
        options: ["CloudWatch 크로스 계정 관측성(Cross-Account Observability)을 설정한다.", "모든 로그를 중앙 S3 버킷으로 전송한다.", "IAM 역할을 사용하여 각 계정에 AssumeRole 한다.", "타사 모니터링 툴을 쓴다."],
    ...
    // Q158: 민감한 DB 마이그레이션 
    {
        ...
        options: ["Amazon RDS for SQL Server (Multi-AZ)를 사용하고 KMS로 암호화한다.", "EC2에 SQL Server를 설치하고 EBS 암호화를 사용한다.", "DynamoDB로 마이그레이션한다.", "S3에 데이터를 덤프한다."],
    ...
    // Q159: 자바 애플리케이션 플랫폼 
    {
        ...
        options: ["AWS Elastic Beanstalk", "AWS Lambda", "Amazon EC2", "Amazon ECS"],
    ...
    // Q160: 크로스 계정 배포 권한 
    {
        ...
        options: ["Prod 계정에 IAM 역할을 생성하고, Dev 계정의 사용자가 이 역할을 맡도록(AssumeRole) 신뢰 정책을 설정한다.", "Prod 계정의 액세스 키를 개발자에게 공유한다.", "두 계정을 합친다.", "Dev 계정의 IAM 사용자에게 Prod 리소스 권한을 직접 준다."],
    ...
    // Q161: 엔드투엔드 암호화 
    {
        ...
        options: ["KMS로 EBS와 Aurora를 암호화하고, ACM 인증서를 ALB에 적용하여 HTTPS를 사용한다.", "모든 데이터를 클라이언트 측에서 암호화한다.", "S3에만 암호화를 적용한다.", "VPN을 사용한다."],
    ...
    // Q162: 지리적 차단 (Geo-blocking) 
    {
        ...
        options: ["AWS WAF의 지리적 일치(Geo match) 규칙을 사용하여 특정 국가를 차단한다.", "보안 그룹에서 해당 국가 IP 대역을 모두 막는다.", "NACL에서 차단한다.", "EC2 인스턴스에서 방화벽을 설정한다."],
    ...
    // Q163: IAM 정책 조건 (Condition) 
    {
        ...
        options: ["Condition 요소에서 `aws:SourceIp`와 `ec2:Region`을 지정한다.", "Resource 요소에 IP를 적는다.", "Principal 요소를 수정한다.", "보안 그룹을 사용한다."],
    ...
    // Q164: 로드 밸런서 인증서 관리 
    {
        ...
        options: ["AWS Certificate Manager (ACM)에서 공인 인증서를 발급받아 ALB에 연결한다.", "EC2 인스턴스마다 인증서를 설치한다.", "Let's Encrypt를 사용하고 스크립트로 갱신한다.", "S3에 인증서를 저장한다."],
    ...
    // Q165: 안전한 원격 DB 접속 
    {
        ...
        options: ["AWS Site-to-Site VPN 연결을 설정한다.", "RDS를 퍼블릭 서브넷으로 옮긴다.", "인터넷 게이트웨이를 사용한다.", "RDS 보안 그룹에서 0.0.0.0/0을 허용한다."],
    ...
    // Q166: 30일 후 보관 스토리지 
    {
        ...
        options: ["EFS 수명 주기 관리(Lifecycle Management)를 활성화하여 EFS Infrequent Access(IA)로 이동한다.", "S3로 데이터를 복사하는 스크립트를 짠다.", "EBS 콜드 HDD로 옮긴다.", "Glacier로 아카이빙한다."],
    ...
    // Q167: DB 자격 증명 자동 교체 
    {
        ...
        options: ["AWS Secrets Manager", "AWS Systems Manager Parameter Store", "S3 암호화 버킷", "DynamoDB"],
    ...
    // Q168: 불규칙한 DB 트래픽 대응 
    {
        ...
        options: ["Amazon Aurora Serverless", "Amazon RDS for MySQL", "DynamoDB On-Demand", "EC2 위 MySQL"],
    ...
    // Q169: 멀티 리전 네트워크 연결 (Transit Gateway) 
    {
        ...
        options: ["AWS Transit Gateway", "VPC Peering", "Direct Connect Gateway 만 사용", "VPN Mesh"],
    ...
    // Q170: 간단한 데이터 처리 (Lambda) 
    {
        ...
        options: ["AWS Lambda", "AWS Glue", "Amazon EMR", "EC2"],
        answer: "AWS Lambda",
    ...
    // Q171: 서드파티 접근 권한 (Cross-Account Role) 
    {
        ...
        options: ["외부 ID를 포함한 크로스 계정 IAM 역할(Cross-Account Role)을 생성하여 제공한다.", "IAM 사용자를 만들어 액세스 키를 준다.", "버킷을 퍼블릭으로 만든다.", "Cognito 자격 증명을 준다."],
    ...
    // Q172: 대규모 데이터 이관 (Snowball) 
    {
        ...
        options: ["AWS Snowball Edge", "AWS Snowmobile", "S3 Transfer Acceleration", "Direct Connect"],
    ...
    // Q173: 글로벌 업로드 가속 
    {
        ...
        options: ["S3 Transfer Acceleration 활성화", "S3 멀티파트 업로드만 사용", "Global Accelerator 사용", "CloudFront PUT 사용"],
    ...
    // Q174: 웹 서버 포트 개방 
    {
        ...
        options: ["보안 그룹과 NACL의 인바운드 규칙에서 0.0.0.0/0의 443 포트를 허용한다.", "아웃바운드 규칙만 설정한다.", "특정 IP만 허용한다.", "모든 포트를 개방한다."],
    ...
    // Q175: 사용자별 비용 보고서 
    {
        ...
        options: ["'CreatedBy' 태그를 활성화하고 AWS Cost Explorer에서 리포트를 생성한다.", "AWS Budgets 알림을 설정한다.", "CloudTrail 로그를 분석한다.", "Athena로 결제 데이터를 쿼리한다."],
        answer: "'CreatedBy' 태그를 활성화하고 AWS Cost Explorer에서 리포트를 생성한다.",
    ...
    // Q177: 간헐적 DB 사용 비용 절감 
    {
        ...
        options: ["Amazon Aurora Serverless", "Aurora Provisioned", "RDS Multi-AZ", "EC2 DB"],
        answer: "Amazon Aurora Serverless",
    ...
    // Q178: 루트 계정 보안 모범 사례 
    {
        ...
        options: ["루트 사용자에게 MFA 활성화 및 액세스 키 삭제(또는 생성 금지)", "루트 사용자를 관리자용으로 사용", "비밀번호를 팀원과 공유", "CloudTrail 끄기"],
    ...
    // Q179: AMI 공유 시 스냅샷 권한 
    {
        ...
        options: ["스냅샷 암호화에 사용된 KMS 키의 사용 권한을 대상 계정에 부여해야 한다.", "스냅샷을 복호화해서 공유한다.", "S3로 내보내서 공유한다.", "AMI를 퍼블릭으로 만든다."],
    ...
    // Q180: S3 데이터 익명화 (Object Lambda) 
    {
        ...
        options: ["S3 Object Lambda를 사용한다.", "데이터를 복제하여 별도 버킷에 마스킹 버전을 저장한다.", "Lambda를 트리거하여 파일을 수정한다.", "클라이언트에서 마스킹한다."],
    ...
    // Q181: 서버리스 문의 양식 처리 
    {
        ...
        options: ["S3(정적 호스팅) + API Gateway + Lambda + SES", "EC2에 LAMP 스택 설치", "Elastic Beanstalk 사용", "Lightsail 사용"],
    ...
    // Q182: 예산 초과 알림 및 조치 
    {
        ...
        options: ["AWS Budgets + SNS(알림) + Service Control Policy (SCP) 적용", "Cost Explorer + CloudWatch Alarm", "Trusted Advisor", "Lambda로 주기적 확인"],
    ...
    // Q183: 고성능 컴퓨팅 배치 (Cluster Placement Group) 
    {
        ...
        options: ["단일 가용 영역(AZ) 내의 클러스터 배치 그룹(Cluster Placement Group)에 배치한다.", "분산 배치 그룹을 사용한다.", "여러 AZ에 분산한다.", "파티션 배치 그룹을 사용한다."],
    ...
    // Q184: EBS 암호화 (생성 시) 
    {
        ...
        options: ["볼륨 생성 시 '암호화(Encrypt this volume)' 옵션을 체크하고 KMS 키를 선택한다.", "볼륨 생성 후 나중에 암호화한다.", "암호화는 스냅샷에서만 가능하다.", "OS 내부에서 암호화 툴을 돌린다."],
        answer: "볼륨 생성 시 '암호화(Encrypt this volume)' 옵션을 체크하고 KMS 키를 선택한다.",
    ...
    // Q185: 웹 애플리케이션 보안 계층 (3-Tier) 
    {
        ...
        options: ["퍼블릭 서브넷에 Application Load Balancer(ALB)를 배치하고, 웹 서버는 프라이빗으로 옮겨 ALB와 통신하게 한다.", "웹 서버에 퍼블릭 IP를 할당한다.", "모든 서버를 퍼블릭에 둔다.", "NAT 게이트웨이로 인바운드 트래픽을 받는다."],
...
// --- 346번 ~ 451번 (마지막) 문제 추가 ---
    ...
    // Q346: 혼합 인스턴스 정책 (Mixed Instances) 
    {
        ...
        options: ["혼합 인스턴스 정책(Mixed Instances Policy) 사용", "스팟 집합(Spot Fleet) 사용", "예약 인스턴스 구매", "시작 템플릿만 사용"],
    ...
    // Q347: 웹/DB 보안 그룹 구성 
    {
        ...
        options: ["웹 서버: 443 포트(0.0.0.0/0) 허용, DB 서버: 3306 포트(웹 서버 보안 그룹 ID) 허용", "웹 서버: 모든 포트 허용, DB: 웹 서버 IP 허용", "DB 서버: 0.0.0.0/0 허용", "모든 서버: 사내 IP만 허용"],
    ...
    // Q348: 비디오 아카이브 긴급 복구 
    {
        ...
        options: ["S3 Glacier (Expedited Retrieval 사용)", "S3 Glacier Deep Archive", "S3 Standard-IA", "S3 One Zone-IA"],
    ...
    // Q349: VPC 피어링 간 보안 그룹 참조 
    {
        ...
        options: ["DB 보안 그룹에서 VPC A의 CIDR 블록을 허용한다.", "DB 보안 그룹에서 VPC A의 보안 그룹 ID를 참조한다.", "모든 트래픽을 허용한다.", "NAT 게이트웨이를 사용한다."],
    ...
    // Q350: 윈도우 파일 공유 스토리지 
    {
        ...
        options: ["Amazon FSx for Windows File Server", "Amazon EFS", "Amazon S3", "EBS"],
        answer: "Amazon FSx for Windows File Server",
    ...
    // Q351: 하이브리드 SMB 파일 공유 
    {
        ...
        options: ["Amazon FSx File Gateway", "S3 File Gateway", "DataSync", "Direct Connect"],
        answer: "Amazon FSx File Gateway",
    ...
    // Q352: 야간 배치 작업 비용 절감 
    {
        ...
        options: ["스팟 인스턴스(Spot Instances)", "온디맨드 인스턴스", "예약 인스턴스", "전용 호스트"],
    ...
    // Q353: 프라이빗 S3 연결 
    {
        ...
        options: ["S3 게이트웨이 VPC 엔드포인트 사용", "NAT 게이트웨이 사용", "VPN 연결", "인터넷 게이트웨이"],
    ...
    // Q354: 3계층 웹 앱 고가용성 
    {
        ...
        options: ["ALB와 Auto Scaling 그룹을 사용하여 다중 AZ에 웹 서버 배포, RDS를 다중 AZ(Multi-AZ)로 설정", "EC2 인스턴스 크기 확장, RDS 읽기 전용 복제본 생성", "S3에 정적 호스팅, DynamoDB 사용", "CloudFront 사용, Route 53 페일오버 설정"],
    ...
    // Q355: 리프트 앤 시프트 마이그레이션 
    {
        ...
        options: ["AWS Application Migration Service (MGN)", "AWS Schema Conversion Tool", "AWS DataSync", "Snowball"],
    ...
    // Q356: 유해 이미지 자동 탐지 
    {
        ...
        options: ["Amazon Rekognition", "Amazon SageMaker", "Amazon Comprehend", "AWS Lambda 직접 구현"],
    ...
    // Q357: 온프레미스 S3 프라이빗 접근 
    {
        ...
        options: ["S3 인터페이스 VPC 엔드포인트 (PrivateLink)", "S3 게이트웨이 VPC 엔드포인트", "NAT 게이트웨이", "Transit Gateway"],
    ...
    // Q358: DB 비밀번호 자동 교체 
    {
        ...
        options: ["AWS Secrets Manager의 자동 교체 기능 사용", "Systems Manager Parameter Store 사용", "Lambda 함수로 직접 구현", "IAM 데이터베이스 인증 사용"],
    ...
    // Q359: 프라이빗 서브넷 인터넷 연결 
    {
        ...
        options: ["퍼블릭 서브넷에 NAT 게이트웨이를 배치하고 라우팅한다.", "프라이빗 서브넷에 NAT 게이트웨이를 배치한다.", "인터넷 게이트웨이를 직접 연결한다.", "Egress-Only 인터넷 게이트웨이를 사용한다(IPv4)."],
    ...
    // Q360: 대규모 데이터 이전 (DataSync) 
    {
        ...
        options: ["AWS DataSync", "AWS Snowmobile", "rsync", "S3 Transfer Acceleration"],
        answer: "AWS DataSync",
    ...
    // Q361: MFA 로그인 강제 
    {
        ...
        options: ["Amazon Cognito User Pools의 MFA 기능 활성화", "IAM 사용자 MFA 사용", "Lambda로 SMS 발송 로직 구현", "타사 인증 솔루션 사용"],
    ...
    // Q362: EBS 스냅샷 장기 보관 비용 절감 
    {
        ...
        options: ["EBS 스냅샷 아카이브(EBS Snapshots Archive) 계층 사용", "S3 Glacier로 스냅샷 복사", "스냅샷을 EC2 이미지로 변환", "S3 Standard에 저장"],
    ...
    // Q363: 선입선출 메시지 처리 
    {
        ...
        options: ["Amazon SQS FIFO 대기열", "Amazon SQS 표준 대기열", "Amazon SNS", "Kinesis Data Streams"],
        answer: "Amazon SQS FIFO 대기열",
    ...
    // Q364: Route 53 다중 응답 라우팅 
    {
        ...
        options: ["다중 값 응답(Multivalue Answer) 라우팅", "단순(Simple) 라우팅", "가중치(Weighted) 라우팅", "지연 시간(Latency) 라우팅"],
    ...
    // Q365: 읽기 집약적 DB 확장 
    {
        ...
        options: ["Amazon Aurora MySQL 및 오토 스케일링 읽기 복제본", "RDS for MySQL 단일 인스턴스", "EC2에 MySQL 클러스터 구축", "Redshift로 이전"],
    ...
    // Q366: 웹/DB 보안 그룹 포트 설정 
    {
        ...
        options: ["웹 SG: 인바운드 443(0.0.0.0/0), DB SG: 인바운드 3306(소스: 웹 SG ID)", "웹 SG: 모든 포트 허용, DB SG: 모든 포트 허용", "웹 SG: 80포트 허용, DB SG: 퍼블릭 IP 허용", "NACL만 사용"],
    ...
    // Q367: 유료 콘텐츠 보안 전송 
    {
        ...
        options: ["CloudFront 서명된 URL(Signed URL) 또는 서명된 쿠키(Signed Cookies)", "S3 버킷 정책으로 IP 제한", "WAF로 접속 차단", "IAM 사용자 생성"],
    ...
    // Q368: 글로벌 게임 UDP 가속 
    {
        ...
        options: ["AWS Global Accelerator", "Amazon CloudFront", "Route 53", "Direct Connect"],
        answer: "AWS Global Accelerator",
    ...
    // Q369: VPC 내부 Lambda S3 접근 
    {
        ...
        options: ["S3용 게이트웨이 VPC 엔드포인트 생성 및 라우팅 테이블 추가", "Lambda 함수에 퍼블릭 IP 할당", "인터넷 게이트웨이 연결", "Lambda 메모리 증설"],
    ...
    // Q370: 무중단 DB 마이그레이션 
    {
        ...
        options: ["AWS DMS(Database Migration Service)와 CDC(변경 데이터 캡처) 사용", "mysqldump로 백업 후 복원", "S3로 데이터 내보내기", "Schema Conversion Tool만 사용"],
    ...
    // Q371: 이미지 썸네일 자동 생성 
    {
        ...
        options: ["S3 이벤트 알림으로 Lambda 함수를 트리거하여 이미지 처리", "EC2 인스턴스로 S3 폴더 감시", "CloudWatch 경보 사용", "Batch 작업 스케줄링"],
    ...
    // Q372: S3 JSON 데이터 분석 
    {
        ...
        options: ["Amazon Athena 사용", "Redshift로 로드", "DynamoDB로 마이그레이션", "EC2에서 grep 사용"],
        answer: "Amazon Athena 사용",
    ...
    // Q373: 로드 밸런서 전송 암호화 
    {
        ...
        options: ["TLS 리스너를 생성하고 ACM 인증서를 NLB에 배포한다.", "KMS로 데이터를 암호화한다.", "WAF를 NLB에 연결한다.", "보안 그룹을 강화한다."],
    ...
    // Q374: DB 연결 폭주 해결 
    {
        ...
        options: ["Amazon RDS Proxy", "ElastiCache", "DynamoDB", "SQS"],
        answer: "Amazon RDS Proxy",
    ...
    // Q375: 복잡한 VPC 연결 단순화 
    {
        ...
        options: ["AWS Transit Gateway 사용", "Direct Connect Gateway", "VPN CloudHub", "Proxy 서버 구축"],
    ...
    // Q376: Lambda 전용선 연결 
    {
        ...
        options: ["Lambda 함수를 VPC에 연결(VPC 설정 구성)한다.", "Lambda에 공인 IP를 할당한다.", "VPN을 Lambda 코드 내에서 연결한다.", "불가능하다."],
    ...
    // Q377: Lambda 콜드 스타트 방지 
    {
        ...
        options: ["Lambda 프로비저닝된 동시성(Provisioned Concurrency) 활성화", "메모리 늘리기", "CloudFront 캐싱", "Keep-Alive 설정"],
    ...
    // Q378: 모바일 앱 데이터 배포 
    {
        ...
        options: ["SNS로 받아 SQS로 팬아웃(Fan-out)하고 Lambda로 가공하여 전달", "하나의 SQS에서 모든 파트너가 읽어가게 함", "Kinesis로 스트리밍", "Step Functions로 순차 처리"],
    ...
    // Q379: CloudTrail 로그 무결성 
    {
        ...
        options: ["로그 파일 무결성 검증(Log File Validation) 활성화", "S3 객체 잠금", "MFA Delete", "KMS 암호화"],
    ...
    // Q380: EKS 볼륨 암호화 
    {
        ...
        options: ["스토리지 클래스에서 암호화된 EBS를 지정하고, EKS 봉투 암호화(Envelope Encryption) 설정을 통해 KMS 키로 시크릿을 암호화한다.", "워커 노드에서 디스크 암호화 수행", "S3에 시크릿 저장", "암호화 기능 없음"],
    ...
    // Q381: EC2 취약점 스캔 
    {
        ...
        options: ["Amazon Inspector", "Amazon GuardDuty", "AWS WAF", "AWS Shield"],
    ...
    // Q382: 라이선스 준수 (전용 호스트) 
    {
        ...
        options: ["전용 호스트(Dedicated Hosts)", "전용 인스턴스(Dedicated Instances)", "온디맨드 인스턴스", "예약 인스턴스"],
    ...
    // Q383: 퍼블릭 액세스 차단 
    {
        ...
        options: ["AWS Organizations의 SCP를 사용하여 S3 퍼블릭 액세스 차단 설정을 해제하지 못하게 막는다.", "모든 버킷 정책을 수동 점검", "CloudWatch 경보 설정", "IAM 사용자 권한 삭제"],
    ...
    // Q384: SaaS 데이터 수집 (AppFlow) 
    {
        ...
        options: ["Amazon AppFlow", "AWS Lambda", "AWS Glue", "DataSync"],
    ...
    // Q385: 실시간 분석 및 쿼리 
    {
        ...
        options: ["스트림 쿼리: Kinesis Data Analytics, S3 쿼리: Amazon Athena", "스트림: Redshift, S3: EMR", "스트림: Lambda, S3: Glue", "스트림: SQS, S3: RDS"],
    ...
    // Q386: 정적 이미지 호스팅 비용 절감 
    {
        ...
        options: ["이미지를 S3 버킷에 저장하고 CloudFront를 통해 제공한다.", "EBS 볼륨을 증설한다.", "EFS를 사용한다.", "ElastiCache를 사용한다."],
    ...
    // Q387: 비용 이상 징후 탐지 
    {
        ...
        options: ["AWS Cost Anomaly Detection", "AWS Budgets", "Cost Explorer", "CloudWatch Billing Alarm"],
    ...
    // Q388: 재해 복구 (RPO 24시간) 
    {
        ...
        options: ["RDS 자동 백업 스냅샷을 다른 리전으로 하루에 한 번 복사한다.", "다른 리전에 읽기 전용 복제본을 만든다.", "Multi-AZ를 활성화한다.", "데이터를 로컬로 다운로드한다."],
    ...
    // Q389: 리전 간 VPC 피어링 
    {
        ...
        options: ["리전 A VPC의 CIDR 블록을 허용한다.", "리전 A 웹 서버의 보안 그룹 ID를 참조한다.", "모든 IP를 허용한다.", "피어링 연결 ID를 참조한다."],
    ...
    // Q390: SQS 기반 오토 스케일링 
    {
        ...
        options: ["대기열의 메시지 수(ApproximateNumberOfMessagesVisible)를 기반으로 대상 추적 조정 정책 사용", "CPU 사용률 기반 조정", "네트워크 트래픽 기반 조정", "일정 기반 조정"],
    ...
    // Q391: 사용자 지연 시간 최소화 (Latency Routing) 
    {
        ...
        options: ["지연 시간(Latency) 라우팅 정책", "지리적 위치(Geolocation) 라우팅 정책", "가중치 기반 라우팅", "단순 라우팅"],
    ...
    // Q392: S3 암호화 비용 절감 (Bucket Key) 
    {
        ...
        options: ["S3 버킷 키(Bucket Key)를 활성화한다.", "암호화를 비활성화한다.", "S3 관리형 키(SSE-S3)로 변경한다.", "객체 크기를 늘린다."],
    ...
    // Q393: Oracle 백업 보존 (AWS Backup) 
    {
        ...
        options: ["AWS Backup을 사용하여 보존 기간을 90일로 설정한 백업 계획을 만든다.", "수동 스냅샷을 찍고 삭제하지 않는다.", "S3로 데이터를 덤프한다.", "DMS를 사용한다."],
    ...
    // Q394: DynamoDB 장기 백업 
    {
        ...
        options: ["AWS Backup의 수명 주기 기능을 사용하여 백업을 콜드 스토리지로 계층화한다.", "DynamoDB TTL 기능을 사용한다.", "S3로 내보낸 후 Glacier로 이동한다.", "수동으로 백업한다."],
    ...
    // Q395: 고성능 공유 스토리지 (EFS Max I/O) 
    {
        ...
        options: ["EFS Max I/O 성능 모드", "EFS General Purpose 성능 모드", "EBS Provisioned IOPS", "S3 Standard"],
    ...
    // Q396: 민감 정보 자동 차단 (Macie & EventBridge) 
    {
        ...
        options: ["Macie 결과를 EventBridge 규칙으로 받아 SNS 주제로 전송한다.", "CloudTrail 로그를 분석한다.", "S3 이벤트 알림을 사용한다.", "Macie 콘솔을 매일 확인한다."],
    ...
    // Q397: Oracle -> Aurora 마이그레이션 
    {
        ...
        options: ["Amazon Aurora PostgreSQL 또는 MySQL", "Amazon RDS for MariaDB", "DynamoDB", "Redshift"],
    ...
    // Q398: 로그 중앙화 (S3 복제) 
    {
        ...
        options: ["S3 동일 리전 복제(SRR) 규칙 설정", "Lambda 함수로 복사", "DataSync 사용", "로그 에이전트 설치"],
    ...
    // Q399: 로드 밸런서 트래픽 불균형 (Sticky Session) 
    {
        ...
        options: ["세션 유지(Sticky Sessions) 기능이 활성화되어 있다.", "라운드 로빈 알고리즘이 켜져 있다.", "상태 확인(Health Check)이 실패했다.", "교차 영역 로드 밸런싱이 꺼져 있다."],
    ...
    // Q400: 리소스 구성 감사 (Config) 
    {
        ...
        options: ["AWS Config", "Trusted Advisor", "CloudTrail", "Macie"],
        answer: "AWS Config",
    ...
    // Q401: 지역 간 이벤트 처리 
    {
        ...
        options: ["리전 A의 S3 이벤트 -> 리전 A의 SNS -> 리전 B의 SQS -> 리전 B의 Lambda", "리전 A에서 리전 B의 Lambda 직접 호출", "S3 복제 사용", "EventBridge 글로벌 버스 사용"],
    ...
    // Q402: 하이브리드 캐시 스토리지 
    {
        ...
        options: ["볼륨 게이트웨이 - 캐시 볼륨(Cached Volumes)", "볼륨 게이트웨이 - 보관 볼륨(Stored Volumes)", "테이프 게이트웨이", "S3 파일 게이트웨이"],
    ...
    // Q403: EKS 공유 파일 시스템 
    {
        ...
        options: ["Amazon EFS", "EBS", "S3", "Instance Store"],
        answer: "Amazon EFS",
    ...
    // Q404: 1일 1특가 사이트 (대규모 트래픽) 
    {
        ...
        options: ["S3(정적) + CloudFront + API Gateway + Lambda + DynamoDB(동적)", "EC2 Auto Scaling + RDS", "Lightsail", "ECS + Fargate"],
    ...
    // Q405: 보고서용 DB 부하 분산 
    {
        ...
        options: ["읽기 전용 복제본(Read Replica)을 생성하여 분석 쿼리를 수행한다.", "Multi-AZ를 활성화한다.", "DB 인스턴스 크기를 늘린다.", "S3로 백업 후 Athena로 쿼리한다."],
    ...
    // Q406: 글로벌 라이브 스트리밍 (TCP/UDP) 
    {
        ...
        options: ["AWS Global Accelerator", "CloudFront", "Route 53", "Direct Connect"],
        answer: "AWS Global Accelerator",
    ...
    // Q407: Aurora I/O 비용 절감 
    {
        ...
        options: ["Aurora I/O-Optimized 구성 사용", "Aurora Standard 사용", "프로비저닝된 IOPS 사용", "인스턴스 크기 축소"],
    ...
    // Q408: Aurora 읽기 확장 자동화 
    {
        ...
        options: ["Aurora Auto Scaling", "EC2 Auto Scaling", "수동 추가", "Lambda로 모니터링"],
    ...
    // Q409: 예산 초과 방지 조치 
    {
        ...
        options: ["AWS Budgets에서 예산 및 경보 설정", "Cost Explorer API 사용", "Billing 콘솔 매일 확인", "Trusted Advisor"],
    ...
    // Q410: 온프레미스 스토리지 전체 백업 
    {
        ...
        options: ["볼륨 게이트웨이 - 보관 볼륨(Stored Volumes)", "캐시 볼륨(Cached Volumes)", "파일 게이트웨이", "테이프 게이트웨이"],
    ...
    // Q411: EBS 암호화 (기존 볼륨) 
    {
        ...
        options: ["볼륨의 스냅샷 생성 -> 스냅샷 복사 시 암호화 옵션 선택 -> 암호화된 스냅샷으로 새 볼륨 생성", "볼륨 속성에서 암호화 체크", "KMS 키를 볼륨에 직접 연결", "불가능함"],
    ...
    // Q412: KMS 키 자동 교체 
    {
        ...
        options: ["KMS 고객 관리형 키(CMK)의 자동 키 교체 활성화", "수동으로 키 생성 및 앱 업데이트", "S3 관리형 키 사용", "CloudHSM 사용"],
    ...
    // Q413: S3 데이터 불변성 (WORM) 
    {
        ...
        options: ["S3 객체 잠금(Object Lock)을 규정 준수(Compliance) 모드로 설정", "거버넌스(Governance) 모드로 설정", "버전 관리 활성화", "MFA Delete 사용"],
        answer: "S3 객체 잠금(Object Lock)을 규정 준수(Compliance) 모드로 설정",
    ...
    // Q414: 단기 불규칙 워크로드 비용 
    {
        ...
        options: ["온디맨드 인스턴스", "스팟 인스턴스", "예약 인스턴스", "Savings Plans"],
        answer: "온디맨드 인스턴스",
    ...
    // Q415: EC2 백업 및 리전 복사 자동화 
    {
        ...
        options: ["AWS Backup", "Data Lifecycle Manager (DLM)", "Lambda + Cron", "S3 복제"],
        answer: "AWS Backup",
    ...
    // Q416: 파이썬 마이크로서비스 
    {
        ...
        options: ["AWS Lambda", "EC2", "EKS", "Elastic Beanstalk"],
        answer: "AWS Lambda",
    ...
    // Q417: 양방향 SMS 마케팅 
    {
        ...
        options: ["Amazon Pinpoint", "Amazon SNS", "SES", "Connect"],
    ...
    // Q418: 3계층 아키텍처 마이그레이션 
    {
        ...
        options: ["웹(퍼블릭), 앱(프라이빗), DB(프라이빗 서브넷의 RDS) 배치", "모두 퍼블릭 서브넷 배치", "DB를 퍼블릭에 배치", "앱과 DB를 같은 서버에 설치"],
    ...
    // Q419: DB 커넥션 풀링 
    {
        ...
        options: ["Amazon RDS Proxy 사용", "Read Replica 추가", "Multi-AZ 사용", "DynamoDB 사용"],
    ...
    // Q420: 결제 시스템 순서 보장 
    {
        ...
        options: ["SQS FIFO 대기열", "SQS 표준 대기열", "SNS", "Kinesis"],
        answer: "SQS FIFO 대기열",
    ...
    // Q421: 일일 리포트 이메일 발송 
    {
        ...
        options: ["EventBridge(스케줄) -> Lambda(집계 및 HTML 생성) -> SES(이메일 발송)", "EC2 크론탭", "CloudWatch 경보", "SNS"],
    ...
    // Q422: 테스트 DB 비용 절감 
    {
        ...
        options: ["RDS 인스턴스 일시 중지(Stop)", "스냅샷 찍고 삭제", "Multi-AZ 해제", "최소 사양으로 변경"],
    ...
    // Q423: 멀티 AZ 파일 시스템 (NetApp ONTAP) 
    {
        ...
        options: ["Amazon FSx for NetApp ONTAP", "EFS", "FSx for Windows", "S3"],
    ...
    // Q424: SFTP 업로드 자동 처리 
    {
        ...
        options: ["Transfer Family로 파일을 받아 S3에 저장하고, S3 이벤트 알림으로 Lambda 트리거", "EC2 폴링", "CloudWatch 이벤트", "Step Functions"],
    ...
    // Q425: 오래된 파일 자동 이동 
    {
        ...
        options: ["S3 수명 주기 정책(Lifecycle Policy)을 설정하여 S3 Standard-IA로 이동", "Intelligent-Tiering", "직접 삭제", "Glacier로 이동"],
    ...
    // Q426: HTTP 플러드 공격 방어 
    {
        ...
        options: ["AWS WAF의 속도 기반 규칙(Rate-based rule) 사용", "NACL", "보안 그룹", "Shield Standard"],
    ...
    // Q427: DB 장애 조치 시간 단축 
    {
        ...
        options: ["RDS Proxy를 사용하여 장애 조치 시 연결 전환 속도를 높인다.", "리전 복제본 사용", "백업 복구", "EC2 DB 사용"],
    ...
    // Q428: CloudFormation 권한 위임 
    {
        ...
        options: ["CloudFormation에 서비스 역할(Service Role)을 할당하고 PassRole 권한을 준다.", "사용자에게 관리자 권한 부여", "루트 계정 사용", "권한 경계 해제"],
    ...
    // Q429: S3 데이터 서버리스 쿼리 
    {
        ...
        options: ["Amazon Athena", "Redshift", "RDS", "DynamoDB"],
        answer: "Amazon Athena",
    ...
    // Q430: 비디오 처리 고성능 I/O 
    {
        ...
        options: ["EFS Max I/O 성능 모드", "General Purpose 모드", "EBS Provisioned IOPS", "S3"],
        answer: "EFS Max I/O 성능 모드",
    ...
    // Q431: ECS 태스크 권한 (Task Role) 
    {
        ...
        options: ["ECS 태스크 역할(Task Role)을 정의하여 태스크 정의에 할당한다.", "인스턴스 프로파일 사용", "액세스 키 하드코딩", "버킷 정책 개방"],
    ...
    // Q432: 클러스터 배치 그룹 
    {
        ...
        options: ["클러스터 배치 그룹(Cluster Placement Group) 사용", "분산 배치 그룹", "파티션 배치 그룹", "전용 호스트"],
    ...
    // Q433: 실시간 데이터 웨어하우징 
    {
        ...
        options: ["Amazon Kinesis Data Firehose", "DataSync", "Glue", "Lambda"],
        answer: "Amazon Kinesis Data Firehose",
    ...
    // Q434: 헬스케어 데이터 보안 (PHI) 
    {
        ...
        options: ["S3 버킷 정책에서 'aws:SecureTransport' 조건으로 HTTPS 강제 및 KMS 암호화 사용", "MFA Delete 사용", "퍼블릭 차단", "Macie 사용"],
        answer: "S3 버킷 정책에서 'aws:SecureTransport' 조건으로 HTTPS 강제 및 KMS 암호화 사용",
    ...
    // Q435: 같은 AZ 내 데이터 전송 무료 
    {
        ...
        options: ["동일한 가용 영역(AZ) 내에 인스턴스를 배치하고 사설 IP로 통신한다.", "다른 AZ에 배치", "퍼블릭 IP 통신", "다른 리전 배치"],
    ...
    // Q436: ALB 쏠림 현상 (Sticky Session) 
    {
        ...
        options: ["대상 그룹에서 고정 세션(Sticky Session)이 활성화되어 있다.", "헬스 체크 실패", "잘못된 라우팅 알고리즘", "인스턴스 크기 차이"],
    ...
    // Q437: SQL Server Always On 가용성 그룹 
    {
        ...
        options: ["Amazon FSx for Windows File Server", "EBS", "S3", "EFS"],
        answer: "Amazon FSx for Windows File Server",
    ...
    // Q438: 장기 보관 아카이브 
    {
        ...
        options: ["Amazon S3 Glacier Deep Archive", "S3 Glacier", "S3 Standard-IA", "S3 Standard"],
    ...
    // Q439: 다국어 음성 분석 파이프라인 
    {
        ...
        options: ["Transcribe(음성->텍스트) -> Translate(번역) -> Comprehend(감정 분석)", "Polly -> Lex -> Rekognition", "Textract -> Translate -> Kendra", "Connect -> Lambda -> SNS"],
    ...
    // Q440: 타 계정 EFS 접근 (VPC 피어링) 
    {
        ...
        options: ["두 VPC 간에 VPC 피어링을 설정한다.", "Transit Gateway 사용", "VPN 연결", "인터넷을 통해 연결"],
    ...
    // Q441: 대용량 데이터 수집 (API Gateway) 
    {
        ...
        options: ["API Gateway + Lambda + DynamoDB", "EC2 + ALB + RDS", "Kinesis + EMR", "S3 + Athena"],
        answer: "API Gateway + Lambda + DynamoDB",
    ...
    // Q442: RDS 보고서 쿼리 분리 
    {
        ...
        options: ["읽기 전용 복제본(Read Replica)을 생성하여 리포트 쿼리를 수행한다.", "Multi-AZ 대기 인스턴스 사용", "S3로 덤프", "ElastiCache 사용"],
    ...
    // Q443: 대용량 DB 마이그레이션 (Snowball + DMS) 
    {
        ...
        options: ["Snowball Edge로 초기 데이터 이전 + DMS로 변경 사항 복제(CDC)", "DataSync만 사용", "VPN 사용", "Direct Connect 설치"],
    ...
    // Q444: 100GB 데이터 전송 (DataSync) 
    {
        ...
        options: ["AWS DataSync", "Snowball", "S3 CLI", "VPN"],
        answer: "AWS DataSync",
    ...
    // Q445: 썸네일 생성 트리거 
    {
        ...
        options: ["S3 이벤트 알림 -> Lambda 함수", "S3 -> SNS -> SQS -> EC2", "CloudWatch 경보 -> Lambda", "EventBridge 스케줄"],
    ...
    // Q446: SQL 인젝션 방어 (WAF) 
    {
        ...
        options: ["AWS WAF를 ALB에 연결하고 SQL 인젝션 차단 규칙 활성화", "보안 그룹 강화", "NACL 설정", "Shield Advanced"],
    ...
    // Q447: CloudFront 인증 (Lambda@Edge) 
    {
        ...
        options: ["Lambda@Edge와 Amazon Cognito 연동", "WAF IP 차단", "S3 버킷 정책", "Geo-blocking"],
    ...
    // Q448: Auto Scaling 지표 (Target Tracking) 
    {
        ...
        options: ["대상 추적(Target Tracking) 조정 정책", "단계 조정 정책", "단순 조정 정책", "예약 조정"],
        answer: "대상 추적(Target Tracking) 조정 정책",
    ...
    // Q449: 임시 쿼리 분석 (Read Replica) 
    {
        ...
        options: ["읽기 전용 복제본을 만들고 분석가는 복제본에서 쿼리하게 한다.", "DB 사양 업그레이드", "ElastiCache 사용", "DynamoDB로 이전"],
    ...
    // Q450: S3 버전 관리 현황 파악 
    {
        ...
        options: ["Amazon S3 Storage Lens", "Config", "CloudWatch", "Macie"],
        answer: "Amazon S3 Storage Lens",
    ...
    // Q451: 필드 레벨 암호화 
    {
        ...
        options: ["CloudFront 필드 레벨 암호화(Field-level encryption) 사용", "HTTPS(SSL) 사용", "VPN 사용", "S3 암호화"],
